...
export default function EditChallengePage() {
    ...
    api.challenges
      ...
      .then((c) => {
        setForm({
          ...
          maxMessages: String(c.maxMessages),
          estimatedMinutes: String(c.estimatedMinutes),
          attemptsAllowed: c.attemptsAllowed != null ? String(c.attemptsAllowed) : '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.brief.trim().length < 10) {
      setSubmitError('Brief must be at least 10 characters.');
      return;
    }
    setSubmitError(null);
    setSubmitting(true);
    try {
      await api.admin.challenges.update(id, {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Failed to update challenge.');
      setSubmitting(false);
    }
  }
  ...
  if (loadError) {
    return (
      <div style={{ padding: 32 }}>
        <div style={{ color: 'var(--d-expert)', fontSize: 13, marginBottom: 16 }}>{loadError}</div>
